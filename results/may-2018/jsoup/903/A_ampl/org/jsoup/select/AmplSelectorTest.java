...
public class AmplSelectorTest {
    @Test(timeout = 120000)
    public void attributeWithBrackets() throws Exception {
        String html = "<div data='End]'>One</div> <div data='[Another)]]'>Two</div>";
        Assert.assertEquals("<div data=\'End]\'>One</div> <div data=\'[Another)]]\'>Two</div>", html);
        ...
        doc.select("div[data='[Another)]]']").first().text();
        doc.select("div[data=\"End]\"]").first().text();
        doc.select("div[data=\"[Another)]]\"]").first().text();
    }
}
