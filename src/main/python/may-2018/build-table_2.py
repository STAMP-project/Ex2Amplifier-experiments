    ...
    for project in projects:
        ...
        for pr_data in project_dataset_json["pullRequests"]:
            for mode in ["A_ampl", "CATG", "JBSE", "I_ampl"]:
                ...
                isGreen = ((nb > 0 and mode == "A_ampl") or nb - sucessfull_aampl[pr_id][0] > 0)
                isGreen_modified = ((nb_modified > 0 and mode == "A_ampl") or nb_modified - sucessfull_aampl[pr_id][2] > 0)

                rows.append(
                    (
                    "\\multirow{4}{*}{"+ str(pr_id) +"}&\\multirow{4}{*}{"+ str(numberOfTestMethodToBeAmplifieds) + "}"
                    if mode == "A_ampl" else " & ") + "&{}&{}&{}&{}&{}&{}&{}\\\\".format(
                        display_mode,
                        ("\\cellcolor[HTML]{009901}" if isGreen else "") + str((numberOfAmplifiedTestMethods if mode == "A_ampl" else numberOfAmplifiedTestMethods -
                                                                               sucessfull_aampl[pr_id][1])),
                        ("\\cellcolor[HTML]{009901}" if isGreen else "") + str((nb if mode == "A_ampl" else nb - sucessfull_aampl[pr_id][0])),
                        ("\\cellcolor[HTML]{009901}" if isGreen else "") + str(time_amplification),
                        ("\\cellcolor[HTML]{009901}" if isGreen_modified else "") + str((
                        numberOfAmplifiedTestMethods_modified if mode == "A_ampl" else numberOfAmplifiedTestMethods_modified -
                                                                                       sucessfull_aampl[pr_id][3])),
                        ("\\cellcolor[HTML]{009901}" if isGreen_modified else "") + str((nb_modified if mode == "A_ampl" else nb_modified - sucessfull_aampl[pr_id][2])),
                        ("\\cellcolor[HTML]{009901}" if isGreen_modified else "") + str(time_amplification_modified),
                    )
                )
            rows.append("\\hline")
